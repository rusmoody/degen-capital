...
export default function ProfileGate({ onClose, onDone }) {
  ...
  // живая проверка уникальности ника (только когда формат валиден)
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl border border-slate-700 bg-slate-900 p-5" onClick={(e) => e.stopPropagation()}>
        <div className="text-amber-300 font-mono text-xs tracking-[0.25em] mb-1">СОЗДАТЬ ПРОФИЛЬ</div>
        <div className="text-slate-400 text-xs mb-4">Профиль-NFT на Base: аватар навсегда, ник уникален. Минт бесплатный (только газ).</div>
        ...
        <div className="mt-4">
          <input
            ...
            placeholder="никнейм (a-z, 0-9, 3-16)"
          ...
          <div className="mt-1 h-4 text-[11px] font-mono">
            {nick && !valid && <span className="text-rose-400">только a-z, 0-9, длина 3–16</span>}
            {valid && nickCheck.isLoading && <span className="text-slate-500">проверка…</span>}
            {available && <span className="text-emerald-400">✓ ник свободен</span>}
            {taken && <span className="text-rose-400">✗ ник занят</span>}
          </div>
        </div>
        ...
        <div className="mt-4 flex gap-2">
          <button onClick={mint} disabled={!available || busy}
            ...
            {busy ? "Минт…" : "Создать профиль"}
          </button>
          <button onClick={onClose} className="rounded-lg border border-slate-700 bg-slate-800 hover:bg-slate-700 px-4 py-2 text-sm">Позже</button>
        </div>
      </div>
    </div>
  );
